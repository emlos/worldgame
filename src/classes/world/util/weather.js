...
export class Weather {
    constructor({ startDate = new Date(), initial = null, rnd = null, seed = rollSeed() } = {}) {
        this._seed = normalizeSeed(seed);
        this._rnd = rnd ?? makeRNG(deriveSeed(this._seed, "transitions"));
    ...
    step(minutes, startDate = this._date) {
        ...
        while (cursor <= endMs - 1) {
            ...
            const next = Weather._nextWeather(
                this._state,
                season,
                this._rnd,
                d.getUTCHours(),
    ...
    computeTemperature(date = this._date) {
        ...
        // Temperature is a pure query. Noise is keyed by timestamp instead of
        // consuming the weather transition stream, so repeated reads are stable.
        const timeKey = new Date(date).getTime();
        const noise01 =
            (keyedRandom01(this._temperatureSeed, `${timeKey}:a`) +
                keyedRandom01(this._temperatureSeed, `${timeKey}:b`) +
                keyedRandom01(this._temperatureSeed, `${timeKey}:c`)) /
            3;
    ...
    static fromJSON(data, { rnd = null, seed = rollSeed() } = {}) {
        const weather = Object.create(Weather.prototype);
        weather._seed = normalizeSeed(seed);
        weather._rnd = rnd ?? makeRNG(deriveSeed(weather._seed, "transitions"));
        weather._temperatureSeed = deriveSeed(weather._seed, "temperature");
        weather._date = new Date(data.date);
